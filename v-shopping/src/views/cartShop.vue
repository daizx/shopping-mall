...
<div class="container">
  <div class="cart">
              ...
              <div class="item-quantity">
                <div class="select-self select-self-open">
                  <div class="select-self-area">
                    ...
                    <a class="input-add" @click="editCart('minu',item)">+</a>
                  </div>
                </div>
              </div>
            </div>
    ...
    <Modal :mdShow="delmodal" @close="closeModal">
      <p slot="message">你确认要删除此商品吗?</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delCart">确认</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="delmodal = false">关闭</a>
      </div>
    </Modal>
    <vfooter></vfooter>
  </div>
...
export default {
    ...
    methods:{
      pushData(){
        this.$http.get('/users/cart').then((response)=>{
          response=response.body;
          this.cartShop=response.result
        })
      },
      delCart(){
